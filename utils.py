...
def gradient_no_abs(input_tensor, direction):
    smooth_kernel_x = tf.compat.v1.reshape(tf.compat.v1.constant([[0, 0], [-1, 1]], tf.compat.v1.float32), [2, 2, 1, 1])
    smooth_kernel_y = tf.compat.v1.transpose(smooth_kernel_x, [1, 0, 2, 3])
    if direction == "x":
        kernel = smooth_kernel_x
    elif direction == "y":
        kernel = smooth_kernel_y
    gradient_orig = tf.compat.v1.nn.conv2d(input_tensor, kernel, strides=[1, 1, 1, 1], padding='SAME')
    grad_min = tf.compat.v1.reduce_min(gradient_orig)
    grad_max = tf.compat.v1.reduce_max(gradient_orig)
...
def gradient(input_tensor, direction):
    ...
    if direction == "x":
        ...
        kernel = smooth_kernel_y
    gradient_orig = tf.compat.v1.abs(tf.compat.v1.nn.conv2d(input_tensor, kernel, strides=[1, 1, 1, 1], padding='SAME'))
    grad_min = tf.compat.v1.reduce_min(gradient_orig)
    grad_max = tf.compat.v1.reduce_max(gradient_orig)
    grad_norm = tf.compat.v1.div((gradient_orig - grad_min), (grad_max - grad_min + 0.0001))
    return grad_norm
...
def blur(x):
    ...
    return tf.compat.v1.nn.depthwise_conv2d(x, kernel_var, [1, 1, 1, 1], padding='SAME')
