...
def lrelu(x, trainbable=None):
    return tf.compat.v1.maximum(x*0.2,x)
...
def upsample_and_concat(x1, x2, output_channels, in_channels, scope_name, trainable=True):
    with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
        ...
        deconv = tf.compat.v1.nn.conv2d_transpose(x1, deconv_filter, tf.compat.v1.shape(x2) , strides=[1, pool_size, pool_size, 1], name=scope_name)

        deconv_output =  tf.compat.v1.concat([deconv, x2],3)
...
def Restoration_net(input_r, input_i, training = True):
    with tf.compat.v1.variable_scope('Denoise_Net', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        out = tf.compat.v1.sigmoid(conv10)
        return out
...
def Illumination_adjust_net(input_i, input_ratio):
    with tf.compat.v1.variable_scope('I_enhance_Net', reuse=tf.compat.v1.AUTO_REUSE):
        input_all = tf.compat.v1.concat([input_i, input_ratio], 3)
        ...
        L_enhance = tf.compat.v1.sigmoid(conv4)
    return L_enhance
